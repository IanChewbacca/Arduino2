...
// Bump behavior
const byte FORWARD_SPEED = 255;   // Define normal speeds
const byte BACKWARD_SPEED = 75;  // and backup/turn speed
const int  TURN_DURATION = 1500;   // Turn length in milliseconds
...
void loop() {
  ...
  if (leftIR < brightLeft ) {          // Too far right, stop left motor
    ...
    motorRight->setSpeed(25);       // Too far right, stop RIGHT motor
