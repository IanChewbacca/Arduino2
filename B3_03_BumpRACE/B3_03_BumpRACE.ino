...
// Define Constants
const byte FORWARD_SPEED = 170;   // Define normal speeds
const byte BACKWARD_SPEED = 254;  // and backup/turn speed
const int  TURN_DURATION = 1250;   // Turn time in milliseconds
...
void loop(){
  ...
  if(digitalRead(LEFT_BUMP_PIN) != HIGH) { // the LEFT side switch was bumped
    motorLeft->setSpeed(100); // Slowly back up and turn to right
    motorRight->setSpeed(220);  
    motorLeft->run(BACKWARD);
    motorRight->run(BACKWARD);
    delay(500);                  // for specified duration
  ...
  /* Then check the right sensor: */
   if(digitalRead(RIGHT_BUMP_PIN) != HIGH) { // the RIGHT side switch was bumped
    ...
    motorLeft->setSpeed(220); // Slowly back up and turn to left
